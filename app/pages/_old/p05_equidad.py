...
def _preparar_datos(df_raw: pd.DataFrame, modelo, pipeline) -> pd.DataFrame | None:
    ...
    try:
        ...
        cols_meta_ok  = [c for c in cols_meta if c in df.columns]
        cols_features = [c for c in df.columns if c not in cols_meta_ok]

        X_prep = pipeline.transform(df[cols_features])
