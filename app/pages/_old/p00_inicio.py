    ...
    for col, pestana in zip(columnas, PESTANAS):
        with col:
            ...
            <div style="
                ...
                gap: 0.4rem;
            ">
                <div style="font-size: 2rem;">{pestana["icono"]}</div>
                <div style="
                    font-weight: bold;
                    font-size: 0.9rem;
                ...
                <div style="
                    ...
                    line-height: 1.3;
                ">{pestana["descripcion"]}</div>
            </div>
            """, unsafe_allow_html=True)

            # Pequeña etiqueta de perfil debajo de cada tarjeta
            st.markdown(f"""
            <div style="
                font-size: 0.7rem;
                color: {COLORES["texto_suave"]};
...
def _nota_metodologica():
    ...
    with st.expander("📋 Nota metodológica — haz clic para ampliar", expanded=False):
        st.markdown(f"""
        <div style="color: {COLORES["texto"]}; font-size: 0.9rem; line-height: 1.7;">

        <strong>Dataset:</strong> 109.568 registros · 30.872 alumnos únicos ·
        Universitat Jaume I · Cursos académicos 2010–2020.

        <br><br>

        <strong>Variable objetivo:</strong> abandono definitivo del grado
        (definición estricta, sin incluir traslados ni cambios de titulación).
        Tasa de abandono en test: <strong>29,2 %</strong>.

        <br><br>

        <strong>Proceso:</strong> ingestión → EDA → ingeniería de características
        (auditoría de leakage) → modelado (21 algoritmos, validación cruzada 5-fold
        estratificada) → interpretabilidad (SHAP, LIME) → esta aplicación.

        <br><br>

        <strong>Modelo final:</strong> Stacking con CatBoost, XGBoost, LightGBM
        y Random Forest como modelos base, y regresión logística como meta-learner.
        AUC = 0.931 · F1 = 0.799.

        <br><br>

        <strong>Limitaciones:</strong> el modelo está entrenado con datos hasta 2020.
        Las predicciones para cohortes posteriores deben interpretarse con cautela.
        Los resultados son orientativos y no deben usarse como único criterio
        de decisión sobre ningún estudiante.
